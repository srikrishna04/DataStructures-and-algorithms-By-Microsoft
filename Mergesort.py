test0 = {'input' : { 'nums' : [2, 5, 1, 7, 0, 6]},'output' : [0, 1, 2, 5, 6, 7]}
test1 = {'input' : { 'nums' : [2, 0,-1, 9, 4,-6]},'output' : [-6,-1, 0, 2, 4, 9]}
test2 = {'input' : { 'nums' : [1, 2, 3, 4, 5, 6]},'output' : [1, 2, 3, 4, 5, 6]}
test3 = {'input' : { 'nums' : [6, 5, 4, 3, 2, 1]},'output' : [1, 2, 3, 4, 5, 6]}
test4 = {'input' : { 'nums' : [2, 1, 1, 6, 3, 3]},'output' : [1, 1, 2, 3, 3, 6]}
test5 = {'input' : { 'nums' : []},'output' : []}
test6 = {'input' : { 'nums' : [2]},'output' : [2]}
test7 = {'input' : { 'nums' : [2, 2, 2, 2, 2, 2]},'output' : [2, 2, 2, 2, 2, 2]}
import random
in_list = list(range(100))
out_list = list(range(100))
random.shuffle(in_list)
test8 = {'input' : { 'nums' : [in_list]},'output' : [out_list]}
tests = [test0, test1, test2, test3, test4, test5, test6, test7, test8]

def insertion_sort(nums):
    nums = list(nums)
    for i in range(len(nums)):
        curr = nums.pop(i)
        j = i-1
        while j>=0 and nums[j]>curr:
            j -= 1
        nums.insert(j+1, curr)
    return nums
for test in tests:
    if insertion_sort(test['input']['nums']) == test['output']:
        print('before sorting' +" "+ str(test['input']['nums']))
        print('after sorting' +" "+ str(test['output']))

def merge_sort(nums):
    if len(nums) <= 1:
        return nums
    mid = len(nums) // 2
    left_side = nums[:mid]
    right_side = nums[mid:]
    left_sorted, right_sorted = merge_sort(left_side), merge_sort(right_side)
    sorted_array = merge(left_sorted, right_sorted)
    return sorted_array
def merge(nums1, nums2):
    merged = []
    i, j = 0,0
    while i< len(nums1) and j<len(nums2):
        if nums1[i] <= nums2[j]:
            merged.append(nums1[i])
            i += 1
        elif nums1[i] > nums2[j]:
            merged.append(nums2[j])
            j += 1
    nums1_rem = nums1[i:]
    nums2_rem = nums2[j:]
    return merged + nums1_rem + nums2_rem
for test in tests:
    if merge_sort(test['input']['nums']) == test['output']:
        print('before sorting' +" "+ str(test['input']['nums']))
        print('after sorting' +" "+ str(test['output']))